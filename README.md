...
- 對應表收錄 **72 條 axe 規則**的台灣準則對應與繁中解說，涵蓋台灣「網站無障礙規範」（110.07 版，對齊 WCAG 2.1）78 條成功準則中**可自動化檢測的 26 條**；其餘準則（鍵盤陷阱、字幕品質、閱讀順序等）本質上需人工檢測，任何自動化工具都無法涵蓋。
- axe 的純最佳實務規則（29 條）獨立顯示為「建議」，不計入違規。
- WCAG 2.2 新增準則的規則（如觸控目標尺寸）觸發時會明確標示「台灣規範尚未採用」。
