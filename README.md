...
- **NVDA 報讀預覽**：每個受影響元素下方顯示模擬 [NVDA](https://www.nvda.org.tw/)（盲用視窗資訊系統，台灣最普及的免費螢幕報讀軟體）會怎麼念出它，例如「（無可朗讀名稱）按鈕」「我同意條款 核取方塊 勾選　必要的」。名稱以 axe 的 accessible name 引擎計算，角色與狀態用詞逐條校對自 NVDA 官方正體中文翻譯（`zh_TW`），讓不會操作報讀軟體的開發者也能「看見」朗讀內容
...
- [x] **報讀預覽（第一階段：單一元素，NVDA）** — 已完成。在受影響元素下方顯示模擬朗讀結果
  （名稱＋角色＋常見狀態）。技術基礎：axe 的 accessible name 計算引擎
  （`axe.commons.text.accessibleText` / `axe.commons.aria.getRole`，需於 `axe.run` 後
  重新 `axe.setup()` 重建虛擬 DOM 樹才可用）＋校對自 NVDA 官方 `zh_TW` 翻譯的角色／狀態對照表
